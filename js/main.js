...
function getComment(){
	$.getJSON("http://syeon0727.cafe24.com:10005/uriel/lotto/getComment.do?callback=?",function(data){
			...
			for(var i = 0 ; i < data.length ; i ++){
				...
				message = message.replace(new RegExp('\\>','g'),'&gt;');
				var $li = $('<li>',{
					html : "<p style='white-space: initial;font-size: 14px;text-overflow: initial;word-break: break-all;'>"+message +"<br><span style='font-weight:bold;'>( "+ date.getFullYear() + "년 " + (date.getMonth()+1)+"월 "+ date.getDate()+"일"+" )</span></p>"
				});
				$('ul').append($li);
			}
			$('ul').listview();
